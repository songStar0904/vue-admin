...
const getHandler = (themeCluster, id) => {
  return () => {
...
export async function handleTheme (val) {
  let themeCluster = getThemeCluster(val.replace('#', ''))
  if (!chalk) {
    await getCSSString(url)
  }

  const chalkHandler = getHandler(themeCluster, 'chalk-style')
